...
def setup_blockchain_connection(rpc_url):
    ...
    if web3.is_connected():
        print(Fore.GREEN + "Terhubung ke Humanity Protocol")
    else:
        print(Fore.RED + "Koneksi gagal.")
...
def claim_rewards(private_key, web3, contract):
    try:
        ...
        if genesis_claimed and not claim_status:
            print(Fore.GREEN + f"Mengklaim hadiah untuk alamat: {sender_address} (Hadiah Genesis sudah diklaim).")
            process_claim(sender_address, private_key, web3, contract)
        elif not genesis_claimed:
            print(Fore.GREEN + f"Mengklaim hadiah untuk alamat: {sender_address} (Hadiah Genesis belum diklaim).")
            process_claim(sender_address, private_key, web3, contract)
        else:
            print(Fore.YELLOW + f"Hadiah sudah diklaim untuk alamat: {sender_address} di epoch {current_epoch}. Melewati.")

    except Exception as e:
        handle_error(e, sender_address)

# Menangani kesalahan khusus untuk kejelasan
def handle_error(e, address):
    error_message = str(e)
    if "Rewards: user not registered" in error_message:
        print(Fore.RED + f"Kesalahan: Pengguna {address} tidak terdaftar.")
    else:
        print(Fore.RED + f"Kesalahan mengklaim hadiah untuk {address}: {error_message}")
...
def process_claim(sender_address, private_key, web3, contract):
    try:
        ...
        print(Fore.RED + f"Kesalahan memproses klaim untuk {sender_address}: {str(e)}")
...
if __name__ == "__main__":
    ...
    # Tampilkan pesan untuk mengikuti Twitter
    print(Fore.CYAN + "Ikuti Twitter @ferdie_jhovie untuk mendapatkan update skrip lebih lanjut.")

    # Loop tak terbatas untuk dijalankan setiap 6 jam
    while True:
        private_keys = load_private_keys('private_keys.txt')
        for private_key in private_keys:
            claim_rewards(private_key, web3, contract)
        
        print(Fore.CYAN + "Menunggu selama 6 jam sebelum menjalankan lagi...")
        time.sleep(6 * 60 * 60)  # Untuk tujuan pengujian, Anda mungkin ingin mengurangi waktu ini